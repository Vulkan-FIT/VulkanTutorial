...
constexpr const uint32_t ApiVersion1_1 = makeApiVersion(0, 1, 1, 0);
constexpr const uint32_t ApiVersion1_2 = makeApiVersion(0, 1, 2, 0);
constexpr const uint32_t ApiVersion1_3 = makeApiVersion(0, 1, 3, 0);
...
// taken from vulkan.hpp and adjusted
...
public:
    ...
    StructureChain(StructureChain const& rhs) noexcept : std::tuple<ChainElements...>(rhs)
    {
      link(&std::get<0>(*this),
           &std::get<0>(rhs),
           reinterpret_cast<BaseOutStructure*>(&std::get<0>(*this)),
           reinterpret_cast<const BaseInStructure*>(&std::get<0>(rhs)));
    }
    StructureChain(StructureChain&& rhs) noexcept : std::tuple<ChainElements...>(std::forward<std::tuple<ChainElements...>>(rhs))
    {
      link(&std::get<0>(*this),
           &std::get<0>(rhs),
           reinterpret_cast<BaseOutStructure*>(&std::get<0>(*this)),
    ...
    StructureChain(const ChainElements&... elems) noexcept : std::tuple<ChainElements...>(elems...)
    {
      link<sizeof...(ChainElements) - 1>();
    }
    StructureChain& operator=(const StructureChain& rhs) noexcept
    {
    ...
    {
      return std::get<ChainElementIndex<0, T, Which, void, ChainElements...>::value>(static_cast<const std::tuple<ChainElements...>&>(*this));
    }
    template <typename T0, typename T1, typename... Ts>
    std::tuple<T0 &, T1 &, Ts &...> get() noexcept
    {
      return std::tie(get<T0>(), get<T1>(), get<Ts>()...);
    }
    template <typename T0, typename T1, typename... Ts>
    std::tuple<const T0&, const T1&, const Ts&...> get() const noexcept
    {
      return std::tie(get<T0>(), get<T1>(), get<Ts>()...);
    }
    ...
    template <typename T = typename std::tuple_element<0, std::tuple<ChainElements...>>::type, size_t Which = 0>
    StructureChain& assign(const T& rhs) noexcept
    {
      T& lhs = get<T, Which>();
    ...
    typename std::enable_if<!std::is_same<ClassType, typename std::tuple_element<0, std::tuple<ChainElements...>>::type>::value || ( Which != 0 ), bool>::type
      isLinked() const noexcept
    {
      return isLinked(reinterpret_cast<const BaseInStructure*>(&get<ClassType, Which>()));
    }
    template <typename ClassType, size_t Which = 0>
    typename std::enable_if<!std::is_same<ClassType, typename std::tuple_element<0, std::tuple<ChainElements...>>::type>::value || ( Which != 0 ), void>::type
      relink() noexcept
    {
      auto pNext = reinterpret_cast<BaseInStructure*>(&get<ClassType, Which>());
      auto& headElement = std::get<0>(static_cast<std::tuple<ChainElements...>&>(*this));
      pNext->pNext      = reinterpret_cast<const BaseInStructure*>(headElement.pNext);
    ...
    typename std::enable_if<!std::is_same<ClassType, typename std::tuple_element<0, std::tuple<ChainElements...>>::type>::value || ( Which != 0 ), void>::type
      unlink() noexcept
    {
      unlink(reinterpret_cast<const BaseOutStructure*>(&get<ClassType, Which>()));
    }
...
protected:
    ...
    bool isLinked(BaseInStructure const* pNext) const noexcept
    {
      const BaseInStructure* elementPtr =
        reinterpret_cast<const BaseInStructure*>(&std::get<0>(static_cast<const std::tuple<ChainElements...>&>(*this)));
      while(elementPtr)
      {
    ...
    typename std::enable_if<Index != 0, void>::type link() noexcept
    {
    ...
    void link(void* dstBase, const void* srcBase, BaseOutStructure* dst, const BaseInStructure* src)
    {
      while(src->pNext)
      {
        std::ptrdiff_t offset = reinterpret_cast<char const *>(src->pNext) - reinterpret_cast<const char*>(srcBase);
        dst->pNext            = reinterpret_cast<BaseOutStructure*>(reinterpret_cast<char*>(dstBase) + offset);
        dst                   = dst->pNext;
        src                   = src->pNext;
      }
      dst->pNext = nullptr;
    ...
    void unlink(const BaseOutStructure* pNext) noexcept
    {
      BaseOutStructure* elementPtr = reinterpret_cast<BaseOutStructure*>(&std::get<0>(static_cast<std::tuple<ChainElements...>&>(*this)));
...
namespace std
{
  ...
  struct tuple_size<vk::StructureChain<Elements...>>
  {
    static constexpr size_t value = std::tuple_size<std::tuple<Elements...>>::value;
  };

  template <std::size_t Index, typename... Elements>
  struct tuple_element<Index, vk::StructureChain<Elements...>>
  {
...
StructureChain<T1, T2...> getPhysicalDeviceProperties2(PhysicalDevice pd)  { StructureChain<T1, T2...> p; funcs.vkGetPhysicalDeviceProperties2(pd, &p.get<PhysicalDeviceProperties2>()); return p; }
inline PhysicalDeviceProperties2 getPhysicalDeviceProperties2(PhysicalDevice pd)  { PhysicalDeviceProperties2 p; p.sType = vk::StructureType::ePhysicalDeviceProperties2; p.pNext = nullptr; funcs.vkGetPhysicalDeviceProperties2(pd, &p); return p; }
